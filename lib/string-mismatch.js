...
exports.diff = function (start, end, precision) {
    var changeData = this.getChanges(start, end, ""),
    ...
    result += (nextThis !== "" || nextS !== "") ? nextThis.diff(nextS, precision) : "";
    return result;
};

exports.getChanges = function (source, changed, m, precision) {
    var isThisLonger = source.length >= changed.length,
        bi = 0,  // base index designating the index of first mismacthing character in both strings
        longer = isThisLonger ? source : changed,
        shorter = isThisLonger ? changed : source;
    ...
    if (shorter !== "") {
        for (var rc = 0; rc < len && sub.sbs.length < precision; rc++) {           // rc -> rotate count, precision -> precision factor
            sub = this.getMatchingSubstring(shorter, longer.rotate(rc), cd.mtc); // rotate longer string 1 char and get substring
